...
class Debugger {
  ...
  #close(connection: ConnectionOwner): void {
    ...
    // Close the buffered client writer too, not just the backend. Only the
    // backend was being closed here, so bufferedWriter.close() never ran and
    // any paced/queued `pendingMessages` backlog stayed retained until GC.
    client?.close();
    backend?.close();
  }
...
function webSocketWriter(ws: ServerWebSocket<unknown>): Writer {
  return {
    write: message => {
      ...
      // a positive byte count (SUCCESS). This was previously collapsed with
      // `!!`, which coerces -1 to `true` -- indistinguishable from success --
      // so bufferedWriter (below) never reacted to backpressure at all. We
      // surface the real tri-state result so bufferedWriter can retry a
