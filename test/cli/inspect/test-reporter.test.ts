...
`,
  ...
  test("flushes pending inspector messages to the frontend before process exit", async () => {
    ...
    // cover the layer-(b) ws.sendText() backpressure tri-state fix. A
    // ws:-transport backpressure regression test is required follow-up (see
    // PR-exit-race.md) and can't be added build-unverified here.
    const testCount = 5;
